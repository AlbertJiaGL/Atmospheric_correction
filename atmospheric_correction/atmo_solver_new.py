...
class solving_atmo_paras(object): 
    ...
    def _helper(self, inp):
        ...
        #xap_H. append(tmp1[self.resample_hx,self.resample_hy])
        #xap_dH.append(tmp2[self.resample_hx,self.resample_hy,:])
        #H, dH    = np.array(H).reshape(self.num_blocks_x, self.num_blocks_y), \
        #           np.array(dH)[:,3:6].reshape(self.num_blocks_x, self.num_blocks_y, 3)
    ...
    def _obs_cost(self, p, is_full = True):
        ...
        for i in range(len(self.xap_emus)):
            H, dH   = self.xap_emus[i].predict(X[i][:, self.mask.ravel()].T, do_unc=False) 
            tmp1    = np.zeros((self.num_blocks_x, self.num_blocks_y))
            tmp2    = np.zeros((self.num_blocks_x, self.num_blocks_y, 3))
            tmp1[self.mask] = H
            tmp2[self.mask, :] = np.array(dH)[:,3:6]   
            ...
            H, dH   = self.xbp_emus[i].predict(X[i, self.mask.ravel()].T, do_unc=False) 
            tmp1    = np.zeros((self.num_blocks_x, self.num_blocks_y))
            tmp2    = np.zeros((self.num_blocks_x, self.num_blocks_y, 3))
            tmp1[self.mask] = H
            tmp2[self.mask, :] = np.array(dH)[:,3:6]   
            #H, dH   = np.array(H).reshape(self.num_blocks_x, self.num_blocks_y), \
            #          np.array(dH)[:,3:6].reshape(self.num_blocks_x, self.num_blocks_y, 3)
            xbp_H. append(tmp1[self.resample_hx,self.resample_hy])
            xbp_dH.append(tmp2[self.resample_hx,self.resample_hy,:])

            H, dH   = self.xcp_emus[i].predict(X[i, self.mask.ravel()].T, do_unc=False) 
            tmp1    = np.zeros((self.num_blocks_x, self.num_blocks_y))
            tmp2    = np.zeros((self.num_blocks_x, self.num_blocks_y, 3))
            tmp1[self.mask] = H
            tmp2[self.mask, :] = np.array(dH)[:,3:6]   
            #H, dH   = np.array(H).reshape(self.num_blocks_x, self.num_blocks_y), \
            #          np.array(dH)[:,3:6].reshape(self.num_blocks_x, self.num_blocks_y, 3)
            xcp_H. append(tmp1[self.resample_hx,self.resample_hy])
            xcp_dH.append(tmp2[self.resample_hx,self.resample_hy,:])
        #import pdb;pdb.set_trace()
