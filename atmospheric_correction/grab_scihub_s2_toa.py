...
from s2_scihub_Angle_resample import resample_s2_angles
...
class read_s2(object):
    ...
    def _read_all(self, done = False):
        fname = []
        for i in self.s2_bands:
            fname.append(glob(self.s2_toa_dir+'/IMG_DATA/*%s.jp2'%i)[0])
    ...
    def get_s2_cloud(self,):
        if len(glob(self.s2_toa_dir+'/cloud.tif'))==0:
            self._read_all(self.done)  
            ...
            g            = gdal.Open(glob(self.s2_toa_dir+'/IMG_DATA/*B01.jp2')[0])
            driver       = gdal.GetDriverByName('GTiff')
            g1           = driver.Create(self.s2_toa_dir+'/cloud.tif', \
            ...
            cloud = gdal.Open(self.s2_toa_dir+\
        ...
        try:
            ...
            mask = gdal.Open(glob(self.s2_toa_dir + '/IMG_DATA/*B04.jp2')[0]).ReadAsArray() >= 1.
        self.cloud_cover = 1. * self.cloud.sum() / mask.sum()

        return self.cloud
        
    def get_s2_angles(self):
        if len(glob(self.s2_toa_dir + '/ANG_DATA/*.tif')) == 14:
            self.saa_sza = [self.s2_toa_dir + '/ANG_DATA/SAA_SZA.tif']
            self.vaa_vza = [self.s2_toa_dir + '/ANG_DATA/VAA_VZA_%s.tif'%i for i in self.bands]
        else:
            resample_s2_angles(self.s2_toa_dir)
            self.saa_sza = [self.s2_toa_dir + '/ANG_DATA/SAA_SZA.tif']
            self.vaa_vza = [self.s2_toa_dir + '/ANG_DATA/VAA_VZA_%s.tif'%i for i in self.bands]
if __name__ == '__main__':
    s2 = read_s2('/data/nemesis/tmp/S2A_MSIL1C_20171201T094341_N0206_R036_T34TDT_20171201T114354.SAFE/GRANULE/L1C_T34TDT_A012760_20171201T094519', \
                  2017, 12, 1, bands = ['B02', 'B03', 'B04', 'B08', 'B11', 'B12'] )
    cm = s2.get_s2_cloud()
    s2.get_s2_angles()
