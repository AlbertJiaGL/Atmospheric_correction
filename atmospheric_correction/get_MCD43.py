...
from proxy import get_good
from functools import partial
from multiprocessing import Pool
from datetime import datetime, timedelta
from modis_tile_cal import get_vector_hv, get_raster_hv

with open('usgs_auth', 'rb') as f:
    auth = tuple(f.read().split('\n'))[:2]

def find_files(aoi, obs_time, temporal_window = 16):
    days   = [(obs_time - timedelta(days = i)).strftime('%Y.%m.%d') for i in np.arange(temporal_window, 0, -1)] + \
             [(obs_time + timedelta(days = i)).strftime('%Y.%m.%d') for i in np.arange(0, temporal_window+1,  1)]
    try:
...
def get_one_tile(tile_date):
    ...
    for i in range(100):
        ...
        fname = [i.split('>')[-1] for i in r.content.split('</') if (i[-3:]=='hdf') & (tile in i)]
        if len(fname) == 1:
            break
        else:
            #print(base + date)
...
def get_mcd43(aoi, obs_time, mcd43_dir = './'):
    ...
    par = partial(daily_vrt, vrt_dir = mcd43_dir)
    p = Pool(len(fnames_dates)) 
    p.map(daily_vrt, fnames_dates)
    p.close()                           
    p.join()
...
if __name__ == '__main__':
    ...
    get_mcd43(aoi, obs_time, mcd43_dir = './')
